...
const buttonHeaderStyle = 'hover:bg-white hover:bg-opacity-60 hover:transition-all hover:duration-200 rounded-[4px] p-2 w-[30px] h-[30px]'
		...
		<>
			<div ref={setNodeRef} style={style} {...attributes}>
				<div className='h-full shadow-sm'>
					...
					{/* Note content */}
					<ContentNote date={date} description={description} onClick={handleShowNote} />
				</div>
			</div>
		</>
	)
...
const HeaderNote = ({ title, typeOfNote, onClick, id }: IHeaderNote) => {
	...
	return (
		<div className={`${NotesColors[typeOfNote || 'houseWork']} w-full rounded-md h-full flex items-center`}>
			...
			<button {...listeners} className={`${buttonHeaderStyle}   mr-3`}>
...
const ContentNote = ({ date, description, onClick }: IContentNote) => {
	return (
		<div onClick={onClick} className={`bg-[#fbfbfb] h-[220px] md:h-[180px] lg:h-[120px] text-sm p-2 flex flex-col justify-between`}>
